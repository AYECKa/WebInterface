...
error_reporting(0);
function oidarray(){
$result = mysql_query('SELECT * FROM tc1mib');
$oid = array();
$type = array();
...
function tableArray(){
    $result = mysql_query('SELECT * FROM tc1tables WHERE forTable = "software" AND row = 1');
    $swRow1 = array();
    while($row = mysql_fetch_assoc($result))
    {
    ...
    //swRow2
    $result = mysql_query('SELECT * FROM tc1tables WHERE forTable = "software" AND row = 2');
    $swRow2 = array();
    while($row = mysql_fetch_assoc($result))
    {
    ...
    //fwRow1
    $result = mysql_query('SELECT * FROM tc1tables WHERE forTable = "fpga" AND row = 1');
    $fwRow1 = array();
    while($row = mysql_fetch_assoc($result))
    {
    ...
    //fwRow2
    $result = mysql_query('SELECT * FROM tc1tables WHERE forTable = "fpga" AND row = 2');
    $fwRow2 = array();
    while($row = mysql_fetch_assoc($result))
    {
    ...
    //BL
    $result = mysql_query('SELECT * FROM tc1tables WHERE forTable = "bl" AND row = 1');
    $bl = array();
    while($row = mysql_fetch_assoc($result))
    {
        $bl[$row['name']] = $row['oid'];
